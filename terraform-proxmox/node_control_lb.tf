...
resource "proxmox_vm_qemu" "k3s_control_node_lb" {
  count       = 2
  name        = "${var.control_node_lb_name}${format("%02d", count.index + 1)}" # Name of the new VM
  target_node = var.control_node_lb_prox_target_node[count.index]               # Proxmox node where the VM will be created
  ...
  memory = var.control_node_lb_ram_size

  cpu {
    cores = var.control_node_lb_cores
    type  = "host"
  }
    ...
    scsi {
      scsi0 {
        disk {
          ...
          size    = var.control_node_lb_disk_size # Disk size in GB
        }
      }
    }
  }
  ...
  ipconfig0  = "ip=${var.control_node_lb_ips[count.index]},gw=${var.gateway_ip}" # Static IPv4 network configuration
