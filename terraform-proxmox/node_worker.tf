resource "proxmox_vm_qemu" "k3s_worker_nodes" {
  ...
  name        = var.worker_node_names[count.index]            # Name of the new VM
    ...
    scsi {
      scsi0 {
        disk {
          ...
          size    = var.worker_node_disk_size # Disk size in GB
        }
      }
    }
  }
  ...
  ipconfig0  = "ip=${var.worker_node_ips[count.index]},gw=${var.gateway_ip}" # Static IPv4 network configuration
